...
class Record
  ...
  update: (fields = [], transaction = null) ->
    updated_at_field = (get @, 'config.opts.updated_at_field')
    if updated_at_field && !(includes fields, updated_at_field)
      fn = (get @.values, updated_at_field)
      (set @.data, updated_at_field, (fn @.type, @.values)) if (isFunction fn)
    _fields = (uniq (union [], fields, (compact [ updated_at_field ])))
